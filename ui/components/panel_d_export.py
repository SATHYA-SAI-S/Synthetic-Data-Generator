...
Fully adaptive: certificate and manifest derive from the live session.
"""
import os
import datetime
...
def render_panel_d():
    """Render one-click export center for datasets, certificates, and manifests."""
    ...
    # ---- Adaptive privacy certificate (no fabricated values) ----
    ...
    verdict_line = ("VERDICT:              PRIVACY CERTIFIED (red-team audit passed)"
                    if certified else
                    f"VERDICT:              {verdict} - review red-team audit before release")

    def fmt(v, nd=4):
        return f"{v:.{nd}f}" if isinstance(v, (int, float)) else str(v or "pending")
...
Privacy Guarantee:    Epsilon = {fmt(eps_spent)}  (Delta = {delta_choice})
Gradient Clip Norm:   C = {fmt(clip_norm, 2)}
Noise Multiplier:     Sigma = {fmt(noise_mult, 2)}
MIA Advantage Score:  {fmt(mia_adv)}  [Threshold: below 0.05]
Attack AUC:           {fmt(st.session_state.get('mia_attack_auc'))}   [Random baseline: 0.5000]
...
"""
    ...
    with col1:
        ...
        st.caption("Reflects this run's actual epsilon spend and red-team verdict.")
    ...
    with col2:
        ...
        st.caption("Comprehensive verification breakdown.")
...
Pipeline Engine:  SYNTHGUARD (Tabular Diffusion + DP-SGD)
Accountant:       Renyi DP Accountant (Gaussian Mechanism)
