...
class HeuristicRiskTierAssigner(AbstractRiskTierAssigner):
    ...
    def assign_tiers(self, df: pd.DataFrame, column_profiles: list = None) -> Dict[str, str]:
        ...
        # M-4 Fix: Downsample for correlation if dataset is very long to save memory
        ...
        # H-3 Fix: Explicit rank mapping instead of string min
        ...
        for col1 in df_sample.columns:
            for col2 in df_sample.columns:
                if col1 != col2:
                    if corr_matrix.loc[col1, col2] > self.correlation_threshold:
                        r1 = rank_map[tiers[col1]]
                        r2 = rank_map[tiers[col2]]
                        ...
                        if tiers[col1] != tighter or tiers[col2] != tighter:
                            log.debug(
                                ...
                                col1, col2, tighter, corr_matrix.loc[col1, col2]
                            )
                        tiers[col1] = tighter
                        tiers[col2] = tighter
